...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Custom metrics from modifications
    if "modification_metrics" in batch.meta_info:
        metrics.update(batch.meta_info["modification_metrics"])

    # Process reward extra info (e.g., reward component breakdowns)
    # These are added to batch.non_tensor_batch by reward managers that return dicts
    if batch.non_tensor_batch is not None:
        # Define keys to skip (already processed above or internal metadata)
        skip_keys = {
            "__num_turns__",  # Processed above in lines 213-217
            "tool_call_counts",  # Processed above in lines 219-223
            "extra_info",  # Internal metadata
            "data_source",  # Internal metadata
            "reward_model",  # Internal metadata
        }
        ...
        for key, values in batch.non_tensor_batch.items():
            ...
            # Skip if this would create duplicate metrics
            # Check if any variant of this key already exists in metrics
            if any(f"/{key}/" in metric_key or metric_key.endswith(f"/{key}") for metric_key in metrics.keys()):
                continue

            # Process numeric reward-related fields (reward components, acc, scores, etc.)
            # Check if it's a numpy array with numeric dtype
            if isinstance(values, np.ndarray) and np.issubdtype(values.dtype, np.number):
                # Only process if we have non-aborted samples
                if values.size > 0:
                    # Convert to float for consistent processing
                    values_float = values.astype(float)
                    # Filter out any NaN or inf values
                    valid_values = values_float[np.isfinite(values_float)]

                    if valid_values.size > 0:
                        metrics[f"custom/{key}/mean"] = float(np.mean(valid_values))
                        metrics[f"custom/{key}/std"] = float(np.std(valid_values))
                        metrics[f"custom/{key}/max"] = float(np.max(valid_values))
                        metrics[f"custom/{key}/min"] = float(np.min(valid_values))
